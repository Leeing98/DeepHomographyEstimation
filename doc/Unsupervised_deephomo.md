...
> 2. 图$V$上每个像素的点实际是从$I_A$中采样出来的。由于之前在DLT阶段由$P_A$和$P_B$得到了图像$I_A$到$I_B$的矩阵$\tilde{H}$，可以计算出$I_B$到$I_A$的矩阵$\tilde{H}\_{inv}$。
> 3. 根根据矩阵$\tilde{H_{inv}}$可以将$V$中的像素(假设坐标为$({u_i}\', {v_i}\')$)映射到$I_A$中的坐标上，则图$V$中每个像素的值就完全是取自$I_A$。
> 4. 由于坐标可能出现浮点型数据，而图像是离散型变量，因此需要
